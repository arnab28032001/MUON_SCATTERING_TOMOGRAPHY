...
# Create dataframes from cluster density and deviation data 
    ...
    xminCon, xmaxCon = 80, 90 # Note that  there is a rotation involved that modifies the geometry 
    yminCon, ymaxCon = 80, 90 # quite significantly (as is apparent from the histograms).
    xminU, xmaxU = 55, 65
    yminU, ymaxU = 25, 35
    xminAir, xmaxAir = 85, 95
    yminAir, ymaxAir = 55, 65
    xminPb, xmaxPb = 55, 65
    yminPb, ymaxPb = 85, 95
    xminSS, xmaxSS = 25, 35
    yminSS, ymaxSS = 55, 65
    ...
    data_All = np.array(df.iloc[xminXmean:xmaxXmean, yminYmean:ymaxYmean])    # Mid Position
    # print('filter :', data)
    ...
    data_Con_d = np.array(df_d.iloc[xminCon:xmaxCon, yminCon:ymaxCon])  # consistent with earlier numbers and works well
    data_U_d = np.array(df_d.iloc[xminU:xmaxU, yminU:ymaxU])  # consistent with earlier numbers and works well
    data_Pb_d = np.array(df_d.iloc[xminPb:xmaxPb, yminPb:ymaxPb])  # consistent with earlier numbers and works well
    data_SS_d = np.array(df_d.iloc[xminSS:xmaxSS, yminSS:ymaxSS])
    data_Air_d = np.array(df_d.iloc[xminAir:xmaxAir, yminAir:ymaxAir])
...
# above.
    ...
    plt.colorbar()
    plt.rc('font' , size = 20)
    plt.savefig('Pb_cluster_sep_10cm_det_120_2days_poca_100um.png', dpi=150, bbox_inches='tight')	
    plt.show()
    plt.close()

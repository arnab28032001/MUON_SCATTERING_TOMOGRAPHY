...
public:
	...
	G4Material* Platinum   = nist->FindOrBuildMaterial("G4_Pt");
	G4Material* Tungsten = nist->FindOrBuildMaterial("G4_W");
           G4Material* Plutonium = nist->FindOrBuildMaterial("G4_Pu");
	G4Material* Brass = nist->FindOrBuildMaterial("G4_BRASS");
	G4Material* SS = nist->FindOrBuildMaterial("G4_STAINLESS-STEEL");

        

        // Rectangular concrete container

        G4double  Det_sizeXY    = parameters.Det_sizeXY *cm;
        G4double  Det_sizeZ     = parameters.Det_sizeZ *cm;
        G4double containerC_X = 50. *cm;
        G4double containerC_Y = 50. *cm;
        G4double containerC_Z = 50. *cm;
        
// Uranium rod
        
        G4double containerU_X = 10. *cm;
        G4double containerU_Y = 10. *cm;
        G4double containerU_Z = 30. *cm;
        
// Lead rod
        
        G4double containerPb_X = 10. *cm;
        G4double containerPb_Y = 10. *cm;
        G4double containerPb_Z = 30. *cm;
    
// SS rod
        
        G4double containerSS_X = 10 *cm;
        G4double containerSS_Y = 10 *cm;
        G4double containerSS_Z = 30 *cm;
        
// Air void rod
        
        G4double containerAir_X = 10 *cm;
        G4double containerAir_Y = 10 *cm;
        G4double containerAir_Z = 30 *cm;

        // World
        G4double world_sizeXY   = Det_sizeXY + 6.0 *cm; // must be greater than Det_sizeXY
        G4double world_sizeZ    = ((parameters.RPCPosZ[0] - parameters.RPCPosZ[5])+5.0)*cm;
	...
	G4ThreeVector blockPos5 = G4ThreeVector(parameters.BlockPos5[0]*cm, parameters.BlockPos5[1]*cm, parameters.BlockPos5[2]*cm);
        ...
        G4Box* reccon_shape1 = new G4Box("conc_rec", containerC_X/2, containerC_Y/2, containerC_Z/2);
	G4Box* reccon_shape2 = new G4Box("uranium_rec", containerU_X/2, containerU_Y/2, containerU_Z/2);
	G4Box* reccon_shape3 = new G4Box("Lead_rec", containerPb_X/2, containerPb_Y/2, containerPb_Z/2);
	G4Box* reccon_shape4 = new G4Box("SS_rec", containerSS_X/2, containerSS_Y/2, containerSS_Z/2);
	G4Box* reccon_shape5 = new G4Box("Air_rec", containerAir_X/2, containerAir_Y/2, containerAir_Z/2);
	
        G4LogicalVolume* logiccont1 = new G4LogicalVolume(reccon_shape1, Concrete, "conc_rec");
        G4LogicalVolume* logiccont2 = new G4LogicalVolume(reccon_shape2, Uranium, "uranium_rec");
        G4LogicalVolume* logiccont3 = new G4LogicalVolume(reccon_shape3, Lead, "Lead_rec");
        G4LogicalVolume* logiccont4 = new G4LogicalVolume(reccon_shape4, SS, "SS_rec");
        G4LogicalVolume* logiccont5 = new G4LogicalVolume(reccon_shape5, Air, "Air_rec");
	

        new G4PVPlacement(0, blockPos1, logiccont1, "conc_rec", logicWorld, false, 0, checkOverlaps);
        new G4PVPlacement(0, blockPos2, logiccont2, "uranium_rec", logiccont1, false, 0, checkOverlaps);
        new G4PVPlacement(0, blockPos3, logiccont3, "Lead_rec", logiccont1, false, 0, checkOverlaps);
        new G4PVPlacement(0, blockPos4, logiccont4, "SS_rec", logiccont1, false, 0, checkOverlaps);
        new G4PVPlacement(0, blockPos5, logiccont5, "Air_rec", logiccont1, false, 0, checkOverlaps);
        ...
        vCon->SetForceSolid(true);  logiccont1->SetVisAttributes(vCon);

        auto* vU = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0, 1.0));
        vU->SetForceSolid(true);    logiccont2->SetVisAttributes(vU);

        auto* vPb = new G4VisAttributes(G4Colour(1.0, 0.0, 0.0, 1.0));
        vPb->SetForceSolid(true);   logiccont3->SetVisAttributes(vPb);

        auto* vSS = new G4VisAttributes(G4Colour(0.0, 0.0, 1.0, 1.0));
        vSS->SetForceSolid(true);   logiccont4->SetVisAttributes(vSS);

        auto* vAir = new G4VisAttributes(G4Colour(0.0, 0.0, 0.0, 1.0));
        vAir->SetForceSolid(true);  logiccont5->SetVisAttributes(vAir);

...
# ---------------- GLOBAL KERNEL ----------------
    ...
    output_All = convolve2d(cluster_density[0], ker_All, mode='same')
...
# ---------------- CONVOLUTION ----------------
    ...
    output_Con = convolve2d(cluster_density[0], ker_Con, mode='same')
    output_U   = convolve2d(cluster_density[0], ker_U, mode='same')
    output_Pb  = convolve2d(cluster_density[0], ker_Pb, mode='same')
    output_SS  = convolve2d(cluster_density[0], ker_SS, mode='same')
    output_Air = convolve2d(cluster_density[0], ker_Air, mode='same')
...
# ---------------- THRESHOLDING ----------------
    PRM_Density = np.where(output_All < All_cutoff, cluster_density[0], np.nan)
    PRM_Deviation = np.where(output_All < All_cutoff, average_deviation, np.nan)

    Con_cutoff += 0.2 * np.std(output_Con)
    U_cutoff   += 0.2 * np.std(output_U)
    Pb_cutoff  += 0.2 * np.std(output_Pb)
    SS_cutoff  += 0.2 * np.std(output_SS)
    Air_cutoff += 0.2 * np.std(output_Air)
    ...
    pred_map = np.zeros_like(cluster_density[0])

    pred_map[Con_cluster==1] = 1
    pred_map[U_cluster==2]   = 2
    pred_map[Pb_cluster==3]  = 3
    pred_map[SS_cluster==4]  = 4
    pred_map[Air_cluster==5] = 5
    
    true_map = np.zeros_like(cluster_density[0])

    true_map[74:91, 76:95] = 1   # Concrete
    true_map[50:69, 20:39] = 2   # Uranium
    true_map[49:69, 81:100] = 3  # Lead
    true_map[18:39, 50:69] = 4   # SS
    true_map[79:89, 54:67] = 5   # Air
    ...
    plt.title("PRM Cluster Density")
    plt.colorbar()
    plt.rc('font' , size = 20)
    plt.text(2,95,f'Accuracy = {accuracy:.3f}',
         fontsize=18,
         color='white',
         bbox=dict(facecolor='black', alpha=0.6))
    plt.savefig('PRM_Cluster_Density_POCA.png', dpi=150, bbox_inches='tight')	
    plt.show()
    plt.close()
    
    plt.imshow(PRM_Deviation.T, origin='lower', cmap='jet', extent=[0,100, 0, 100] )#vmax = -300, vmin = -1500)
    #plt.title('PRM density')
    plt.title("PRM Average Deviation")
    plt.colorbar()
    plt.rc('font' , size = 20)
    plt.text(2,95,f'Accuracy = {accuracy:.3f}',
         fontsize=18,
         color='white',
         bbox=dict(facecolor='black', alpha=0.6))
    plt.savefig('PRM_Average_Deviation_POCA.png', dpi=150, bbox_inches='tight')	
    plt.show()
    plt.close()
    
    plt.imshow(Con_cluster.T, origin='lower', cmap='jet', extent=[0,100, 0, 100])#, vmin = 130000, vmax = 165000)
    plt.title('Concrete_remove')
    plt.colorbar()
    plt.rc('font' , size = 20)
...
# Air filter
 ...
 # Convolute each filter with itself to find different cut-offs
    ...
    #plt.savefig("All_cluster_output.png")
    plt.show()
    ...
    #plt.savefig("Air_cluster_output.png")
    plt.show()
...
# above.
    ...
    Con_cluster = Con_cluster.astype(float)
    Con_cluster[Con_cluster <= 0] = np.nan
    ...
    U_cluster=U_cluster.astype(float)
    U_cluster[U_cluster <= 0] = np.nan
    ...
    Pb_cluster=Pb_cluster.astype(float)
    Pb_cluster[Pb_cluster <= 0] = np.nan
    ...
    SS_cluster=SS_cluster.astype(float)
    SS_cluster[SS_cluster <= 0] = np.nan
    ...
    Air_cluster=Air_cluster.astype(float)
    Air_cluster[Air_cluster <= 0] = np.nan
    ...
    plt.imshow(Con_cluster.T, origin='lower', cmap='jet', extent=[0,100, 0, 100])#, vmin = 130000, vmax = 165000)
    #plt.title('Concrete_remove_cluster')
    plt.colorbar()
    plt.rc('font' , size = 20)
    plt.savefig('concrete_cluster_sep_10cm_det_120_2days_poca_100um.png', dpi=150, bbox_inches='tight')	
    plt.show()
    ...
    plt.imshow(U_cluster.T, origin='lower', cmap='jet', extent=[0,100, 0, 100] )# vmin = 440000, vmax = 550000)#vmax = -300, vmin = -1500)
    #plt.title('Uranium_remove_cluster')
    plt.colorbar()
    plt.rc('font' , size = 20)
    plt.savefig('uranium_cluster_sep_10cm_det_120_2days_poca_100um.png', dpi=150, bbox_inches='tight')	
    plt.show()
